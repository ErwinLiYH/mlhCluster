...
def two_layers_cluster(linkage, outer_distance_threshold, inner_distance_threshold):
...
def two_layer_eval(vecs, linkage_metric="cosine", linkage_method="average", start=0, end=1, step=0.001, figsize=(10,16), img_path=None, dpi=600):
    ...
    for i in np.arange(start,end,step):
        outer_relation, inner_relation = two_layers_cluster(linkage_matrix, i, 1)
        num_of_clusters = len(outer_relation)
        if 2<=num_of_clusters<=shape_of_linkage[0]:
            flated_dict, _ = two_layers_flat(outer_relation, inner_relation, shape_of_linkage[0]+1, verbose=False)
    ...
    elbow_locator = KneeLocator(outer_num, outer_distortion_score, curve="convex", direction="decreasing")
    ...
    for i in np.arange(outer_distance+step,end,step):
        outer_relation, inner_relation = two_layers_cluster(linkage_matrix, outer_distance, i)
    ...
    elbow_locator = KneeLocator(inner_num, inner_distortion_score, curve="convex", direction="decreasing")
